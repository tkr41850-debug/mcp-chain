...
| 6-3-lockshared | 2 | CORE-01 | 2 | `TestStatus_Concurrent10WithinOneSecond` — 10 parallel `status <id>` processes complete in <2 s wall-clock (LOCK_SH path). Bound raised from 1 s after post-merge review observed 1.26 s under full `-race` suite load (research §OQ-2 pre-authorized). Isolated runs measure 500–700 ms. | integration | `go test -race -tags=integration ./internal/cli/ -run TestStatus_Concurrent10WithinOneSecond -count=1` | ⬜ |
